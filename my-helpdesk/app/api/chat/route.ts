...
export async function POST(req: Request) {
  const { messages, ticketId }: { messages: ChatMessage[]; ticketId?: number } = await req.json();
  ...
  const result = streamText({
    ...
    async onFinish({ text }) {
      ...
      if (ticket?.subject === 'New Conversation') {
        ...
        const firstText = firstUserMsg?.parts?.find((p) => p.type === 'text')?.text ?? '';
        if (firstText) {
          await prisma.ticket.update({
            where: { id: tid },
            data: { subject: firstText.slice(0, 80) },
          });
        }
      }
    },
